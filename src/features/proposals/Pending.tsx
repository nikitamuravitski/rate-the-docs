...
const PendingProposals = () => {
  ...
  } = trpc.documentation.getPendingProposals.useQuery({
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor('voteSummary', {
      ...
      cell: info => <Vote
        ...
        currentUserVote={info.row.original.votes}
        initialData={info.getValue()}
      />,
      enableSorting: true,
    }),
    columnHelper.display({
      id: 'approve',
      cell: props => <div key={props.row.original.id} className='flex justify-around'>
  ...
  const table = useReactTable({
    ...
    pageCount: proposalsData?.totalPages ?? 0,
    ...
    state: {
      sorting,
      columnVisibility: {
        approve: session?.user?.role === 'admin'
      },
      pagination: {
        pageSize,
        pageIndex
